...
def load_data(fname='data/gps_data/gps_points.csv', BBOXES=None):
	...
	with open(fname, 'r') as f:
		...
		for line in f:
			...
			if not is_point_in_bboxes((float(lon), float(lat)), BBOXES):
				continue
...
def create_trajectories(INPUT_FILE_NAME='data/gps_data/gps_points_07-11.csv', waiting_threshold=5, BBOXES=None):
	"""
	...
	data_points, raw_points, points_tree = load_data(fname=INPUT_FILE_NAME, BBOXES=BBOXES)
...
def draw_simple_roadnet(rn, showNodes=True):
...
def draw_roadnet(rn, clusters, new_clusters=None, paths=None, points=None, showNodes=True):
	...
	if showNodes == True:
		plt.scatter([node.get_lonlat()[0] for node in clusters], [node.get_lonlat()[1] for node in clusters], s=20)

	if paths:
		lines = []
		for path in paths:
			# lines += [[path[i], path[i + 1]] for i in range(len(path) - 1)]
			plt.scatter([x for x, y in path], [y for x, y in path], c='red', s=40)
		# lc = mc.LineCollection(lines, colors='red', linewidths=2)
		# ax.add_collection(lc)

	if points:
		plt.scatter([pt.lon for pt in points], [pt.lat for pt in points], c='green', s=20)

	if new_clusters:
		X, Y = [], []
		for pt in [clusters[c].get_lonlat() for c in new_clusters]:
			X.append(pt[0])
			Y.append(pt[1])
...
def densify_graph(input_graph, densification_rate=50):

	for edge in input_graph.edges():
		intermediate_nodes = partition_simple_edge(edge, distance_interval=densification_rate)
		if len(intermediate_nodes) == 2:
			continue
		input_graph.add_path(intermediate_nodes)
		input_graph.remove_edge(*edge)
	return input_graph


def find_middle_edge(edge):
	"""
	Return the point in the middle of an edge along with its bearing angle.
	:param edge: source, target (in lon, lat)
	:return: triplet (mid_lon, mid_lat, mid_ang)
	"""
	...
	new_lon = (lon1 + lon2) / 2
	new_lat = (lat1 + lat2) / 2
	return (new_lon, new_lat, bearing)

def transform_osm_to_our_graph_format(osm_rn, densification_rate=10):
	"""
	Transform osm graph into our format of graphs in which we infer angles for nodes.
	The process is the following:
	1. densify the graph
	2. find middle points of each edge and compute its angle.
	4. find adjacent edges for linkage
	:param osm_rn: input osm roadnetwork
	:return: a bunch of data structures.
	"""

	print 'densifying initial osm graph'
	densified_osm = densify_graph(osm_rn, densification_rate)
	new_edge_dict = dict()

	print 'finding middle nodes for all edges'
	for edge in densified_osm.edges():
		new_edge_dict[edge] = find_middle_edge(edge)
		# mid_lon, mid_lat, mid_angle = find_middle_edge(edge)

	clusters = []
	clusters_lonlat = []
	list_of_edges = []
	edge_weight = []
	new_roadnet = nx.DiGraph()
	now_ts = datetime.datetime.now()

	# Add new nodes
	print 'adding new nodes'
	for sid, edge in enumerate(new_edge_dict.keys()):
		slon, slat, sang = new_edge_dict[edge]
		new_cluster = Cluster(cid=int(sid), nb_points=1, last_seen=now_ts, lat=slat, lon=slon, angle=sang)
		clusters.append(new_cluster)
		clusters_lonlat.append((slon, slat))
	new_roadnet.add_nodes_from(range(len(clusters)))

	# Add edges to these nodes
	print 'adding new edges'
	for (s, t), (slon, slat, sang) in new_edge_dict.iteritems():
			clus_id = clusters_lonlat.index((slon,slat))
			successors = osm_rn.successors(t)
			for succ in successors:
				sslon, sslat, ssang = new_edge_dict[(t, succ)]
				s_clus_id = clusters_lonlat.index((sslon, sslat))
				new_roadnet.add_edge(clus_id, s_clus_id)
				list_of_edges.append([clusters[clus_id].get_lonlat(), clusters[s_clus_id].get_lonlat()])
				edge_weight.append(0)

	print 'nb edges:', new_roadnet.number_of_edges()
	return clusters, clusters_lonlat, new_roadnet, list_of_edges, edge_weight
...
def create_gps_stream_from_data(DATA_PATH, FILE_CODE, BBOXES=None):
	"""
	...
	"""
	# Generate Trajectories
	trajectories = create_trajectories(INPUT_FILE_NAME='%s/%s.csv' % (DATA_PATH, FILE_CODE), waiting_threshold=21, BBOXES=BBOXES)
...
def kharitaStar_on_imputedOSM(parameters):
	"""
	This version of kharita star takes the initial roadnet as a parameter.
	Date: 16/07/2017
	return a road network from trajectories
	:param trajectories:
	:return:
	...
	original_osm_clusters_lonlats = parameters['original_osm_clusters_lonlats']
	roadnet = parameters['roadnet']
	list_of_edges = parameters['list_of_edges']
	edge_weight = parameters['edge_weight']
	gps_point_stream = parameters['gps_point_stream']
	...
	return roadnet, clusters, matched_osm_clusters, new_osm_clusters
...
def removePathsOSM(osm_rn, nb_paths):
	...
	while len(paths) < nb_paths:
		...
		path.append(next_node)
		paths.append(path)
		osm_rn.remove_edges_from([(path[i], path[i+1]) for i in range(len(path)-1)])
...
if __name__ =='__main__':
	...
	# draw_roadnet(osm_g, showNodes=True)
	# dens_osm_g = densify_graph(osm_g, densification_rate=20)
	# draw_roadnet(dens_osm_g, showNodes=True)
	...
	emputed_osm, paths = removePathsOSM(osm_g, 10)
